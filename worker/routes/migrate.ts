...
import type { Env, BulkMigrateParams, MigrationResult, KVKeyInfo } from '../types';
import { createCfApiRequest, getD1Binding, generateJobId } from '../utils/helpers';
...
async function getAllNamespaceKeys(
    ...
    try {
        do {
            ...
            if (!listResponse.ok) {
                return { keys: [], error: `Failed to list keys: ${listResponse.status}` };
            }
        ...
        return {
            keys: [],
            error: error instanceof Error ? error.message : 'Unknown error listing keys'
        };
    }
}
...
async function migrateKeys(
    ...
    if (!sourceNamespaceId || !targetNamespaceId) {
        return new Response(
            JSON.stringify({ success: false, error: 'sourceNamespaceId and targetNamespaceId are required' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
    }
    ...
    if (sourceNamespaceId === targetNamespaceId) {
        return new Response(
            JSON.stringify({ success: false, error: 'Source and target namespace cannot be the same' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
    }
    ...
    if (!keys || keys.length === 0) {
        return new Response(
            JSON.stringify({ success: false, error: 'keys array is required and must not be empty' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
    }

    logInfo(`Starting key migration: ${keys.length} keys from ${sourceNamespaceId} to ${targetNamespaceId}`,
    ...
    if (isLocalDev) {
        const mockResult: MigrationResult = {
            ...
            errors: 0,
            verification: { passed: true, sourceKeyCount: keys.length, targetKeyCount: keys.length },
            warnings: []
        };
    ...
    try {
      ...
      `).bind(jobId, sourceNamespaceId, keys.length, userEmail).run();
        }
        ...
        await logError(env, error instanceof Error ? error : String(error),
            createErrorContext('migrate', 'migrate_keys_error', { userId: userEmail }), isLocalDev);

        return new Response(
            JSON.stringify({ success: false, error: 'Failed to start migration job' }),
...
async function migrateNamespace(
    ...
    logInfo(`Starting full namespace migration from ${sourceNamespaceId} to ${targetNamespaceId}`,
        createErrorContext('migrate', 'migrate_namespace_start', { userId: userEmail }));
    ...
    if (isLocalDev) {
        ...
        return new Response(JSON.stringify({ jobId: 'mock-job-id', ...mockResult }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
    }

    // Get all keys from source namespace
    const { keys: sourceKeys, error: listError } = await getAllNamespaceKeys(sourceNamespaceId, env, preserveTTL);

    if (listError) {
        return new Response(
            JSON.stringify({ success: false, error: listError }),
            { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
    }
    ...
    if (sourceKeys.length === 0) {
        return new Response(
            JSON.stringify({ success: false, error: 'Source namespace has no keys to migrate' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
    }
    ...
    try {
      ...
      `).bind(
                ...
                keyNames.length,
                userEmail,
                JSON.stringify({ targetNamespaceId, cutoverMode, fullNamespace: true })
            ).run();
        }
        ...
        const params: BulkMigrateParams & { jobId: string; keyExpirations?: Record<string, number> } = {
            jobId,
            sourceNamespaceId,
            targetNamespaceId,
            keys: keyNames,
        ...
        if (preserveTTL) {
            const expirations = Object.fromEntries(
                sourceKeys.filter(k => k.expiration !== undefined).map(k => [k.name, k.expiration as number])
            );
        ...
        await logError(env, error instanceof Error ? error : String(error),
            createErrorContext('migrate', 'migrate_namespace_error', { userId: userEmail }), isLocalDev);
