...
import { logInfo, logWarning, logError, createErrorContext } from './error-logger';
...
export async function validateAccessJWT(request: Request, env: Env): Promise<string | null> {
  ...
  if (!token) {
    ...
    if (cookies) {
      ...
      if (match) {
        ...
        if (matchedToken) {
          token = matchedToken;
          logInfo('JWT token found in cookie', createErrorContext('auth', 'extract_token'));
        }
      }
    }
  } else {
    logInfo('JWT token found in header', createErrorContext('auth', 'extract_token'));
  }

  if (!token) {
    logWarning('No JWT token found in request headers or cookies', createErrorContext('auth', 'extract_token', {
  ...
  try {
    ...
    logInfo('Validating JWT', createErrorContext('auth', 'validate_jwt', {
    ...
    if (!email || typeof email !== 'string') {
      logWarning('JWT payload missing email', createErrorContext('auth', 'validate_jwt'));
      return null;
    }

    logInfo('JWT validated successfully', createErrorContext('auth', 'validate_jwt', {
      userId: email
    }));
    return email;
